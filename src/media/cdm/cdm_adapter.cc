...
void CdmAdapter::Decrypt(StreamType stream_type,
  ...
  // We assume that buffer is decrypted as svp buffer format.
  if (key_system_ == kWidevineKeySystem && stream_type == Decryptor::kVideo) {
    decrypted_buffer->set_use_svp_serialized_data(true);
  }
