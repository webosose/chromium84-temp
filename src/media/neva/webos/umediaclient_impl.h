...
class UMediaClientImpl : public WebOSMediaClient,
 ...
 public:
  ...
  bool onEndOfStream(bool isForward) override;
#else
...
#if defined(USE_TV_MEDIA)
  bool onSeekableRanges(
      const struct uMediaServer::seekable_ranges_info_t&) override;
  bool onSetMasterClockResult(
      const struct uMediaServer::master_clock_info_t&) override;
  bool onSetSlaveClockResult(
      const struct uMediaServer::slave_clock_info_t&) override;
  bool onEncrypted(const struct uMediaServer::encrypted_info_t&) override;
  bool onWaitingForKey() override;
#endif
...
#if defined(USE_TV_MEDIA)
  void DispatchSeekableRanges(
      const struct uMediaServer::seekable_ranges_info_t&);
  void DispatchSetMasterClockResult(
      const struct uMediaServer::master_clock_info_t&);
  void DispatchSetSlaveClockResult(
      const struct uMediaServer::slave_clock_info_t&);
  void DispatchEncrypted(const struct uMediaServer::encrypted_info_t&);
  void DispatchWaitingForKey();
#endif
#if defined(USE_SIGNAGE_MEDIA)
 ...
 private:
  ...
  bool use_force_play_on_same_rate_ = false;
