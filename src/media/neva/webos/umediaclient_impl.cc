...
const char kPhotoVideo[] = "com.webos.app.photovideo";
const char kStoreDemo[] = "com.webos.app.store-demo";
const char kPandora[] = "pandora.lgerp.app";
...
UMediaClientImpl::~UMediaClientImpl() {
  ...
  uMediaServer::uMediaClient::stop();
...
bool UMediaClientImpl::SelectTrack(const MediaTrackType type,
  ...
  std::string t = type == MediaTrackType::kAudio ? "audio" : "video";
  return uMediaServer::uMediaClient::selectTrack(t, audio_track_ids_[id]);
#endif
  NOTIMPLEMENTED();
  return false;
}
...
void UMediaClientImpl::Suspend(SuspendReason reason) {
  ...
  is_suspended_ = true;
#if defined(USE_TV_MEDIA)
  force_unload = reason == SuspendReason::kSuspendedByPolicy;
#endif
...
bool UMediaClientImpl::IsRecoverableOnResume() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
#if defined(USE_TV_MEDIA)
  return !IsReleasedMediaResource();
#else
  return true;
#endif
}
...
#if defined(USE_TV_MEDIA)
  // photo&video and store-demo use legacy preload parameter.
  // So prevent SetPreload by using preload of media element.
  if (IsAppName(kPhotoVideo) || IsAppName(kStoreDemo)) {
    VLOG(1) << "not supported preload. app_id_: " << app_id_;
    return;
  }
#endif
...
bool UMediaClientImpl::IsSupportedPreload() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
#if defined(USE_TV_MEDIA)
  // preload is not supported in pandora app
  if (IsAppName(kPandora))
    return false;
...
bool UMediaClientImpl::CheckUseMediaPlayerManager(
  ...
  // photo&video and store app unuse mediaplayer manger
  if (IsAppName(kPhotoVideo) || IsAppName(kStoreDemo))
    return false;
...
// TODO(neva) Do we still need this even we call this callback when received
// loadingCompleted ?
#if defined(USE_SIGNAGE_MEDIA)
  if (event_listener_)
...
#if defined(USE_TV_MEDIA)
bool UMediaClientImpl::onEndOfStream(bool isForward) {
  main_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&UMediaClientImpl::DispatchEndOfStream, weak_ptr_, isForward));
  return true;
}
#else
...
#if defined(USE_TV_MEDIA)
  playback_rate_on_eos_ = isForward ? 1.0f : -1.0f;
  if ((playback_rate_ > 0.0f && !isForward) ||
      (playback_rate_ < 0.0f && isForward)) {
    FUNC_LOG(1) << " ignore eos. rate: " << playback_rate_
                << ", isForward: " << isForward;
    ignore_eos = true;
  }
#else
...
#if defined(USE_TV_MEDIA)
    status = media::PIPELINE_ERROR_READ;
#else
...
#if defined(USE_TV_MEDIA)
bool UMediaClientImpl::onSeekableRanges(
    const struct uMediaServer::seekable_ranges_info_t& rangesInfo) {
  main_task_runner_->PostTask(
      FROM_HERE, base::Bind(&UMediaClientImpl::DispatchSeekableRanges,
                            weak_ptr_, rangesInfo));
  return true;
}

void UMediaClientImpl::DispatchSeekableRanges(
    const struct uMediaServer::seekable_ranges_info_t& rangesInfo) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  base::AutoLock auto_lock(lock_);
  seekable_ranges_.clear();
  for (int i = 0; i < rangesInfo.numRanges; i++) {
    seekable_ranges_.Add(
        base::TimeDelta::FromMilliseconds(rangesInfo.seekableRanges[i].start),
        base::TimeDelta::FromMilliseconds(rangesInfo.seekableRanges[i].end));
  }
}

media::Ranges<base::TimeDelta> UMediaClientImpl::GetSeekableTimeRanges() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  // support only seekableRange event on specific player
  if (!IsSupportedSeekableRanges()) {
    media::Ranges<base::TimeDelta> ranges;
    if (seekable_)
      ranges.Add(base::TimeDelta(), base::TimeDelta::FromSecondsD(duration_));
    return ranges;
  }

  base::AutoLock auto_lock(lock_);
  return seekable_ranges_;
}

bool UMediaClientImpl::onSetMasterClockResult(
    const struct uMediaServer::master_clock_info_t& masterClockInfo) {
  main_task_runner_->PostTask(
      FROM_HERE, base::Bind(&UMediaClientImpl::DispatchSetMasterClockResult,
                            weak_ptr_, masterClockInfo));
  return true;
}

void UMediaClientImpl::DispatchSetMasterClockResult(
    const struct uMediaServer::master_clock_info_t& masterClockInfo) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  FUNC_LOG(2);
  if (IsRequiredUMSInfo() && event_listener_) {
    event_listener_->OnUMSInfoUpdated(
        MasterClockInfoToJson(MediaId(), masterClockInfo));
  }
}

bool UMediaClientImpl::onSetSlaveClockResult(
    const struct uMediaServer::slave_clock_info_t& slaveClockInfo) {
  main_task_runner_->PostTask(
      FROM_HERE, base::Bind(&UMediaClientImpl::DispatchSetSlaveClockResult,
                            weak_ptr_, slaveClockInfo));
  return true;
}

void UMediaClientImpl::DispatchSetSlaveClockResult(
    const struct uMediaServer::slave_clock_info_t& slaveClockInfo) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  FUNC_LOG(2);
  if (IsRequiredUMSInfo() && event_listener_) {
    event_listener_->OnUMSInfoUpdated(
        SlaveClockInfoToJson(MediaId(), slaveClockInfo));
  }
}

bool UMediaClientImpl::onEncrypted(
    const struct uMediaServer::encrypted_info_t& encryptedInfo) {
  main_task_runner_->PostTask(
      FROM_HERE, base::Bind(&UMediaClientImpl::DispatchEncrypted, weak_ptr_,
                            encryptedInfo));
  return true;
}

void UMediaClientImpl::DispatchEncrypted(
    const struct uMediaServer::encrypted_info_t& encryptedInfo) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  FUNC_LOG(2) << " data size=" << encryptedInfo.initData.size();
  if (event_listener_) {
    event_listener_->OnEncryptedMediaInitData(encryptedInfo.initDataType,
                                              encryptedInfo.initData);
  }
}

bool UMediaClientImpl::onWaitingForKey() {
  main_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&UMediaClientImpl::DispatchWaitingForKey, weak_ptr_));
  return true;
}

void UMediaClientImpl::DispatchWaitingForKey() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  FUNC_LOG(1);
  if (event_listener_)
    event_listener_->OnWaitingForDecryptionKey();
}
#endif

#if defined(USE_SIGNAGE_MEDIA)
bool UMediaClientImpl::onFocusChanged(bool value) {
  main_task_runner_->PostTask(
      FROM_HERE,
      base::Bind(&UMediaClientImpl::DispatchFocusChanged, weak_ptr_));
  return true;
}
...
void UMediaClientImpl::DispatchFocusChanged() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  if (event_listener_) {
    event_listener_->OnAudioFocusChanged();
  }
}
#endif
...
std::string UMediaClientImpl::UpdateMediaOption(const std::string& mediaOption,
  ...
  // add mediaFormat.type = "livetv"
  if (IsAppName(kLiveDmost))
    media_option["option"]["mediaFormat"]["type"] = "livetv";
#if defined(HYBRIDCAST)
  if (IsAppName(kLiveHybridcast)) {
    FUNC_LOG(1) << " - type = game_media";
    media_option["option"]["mediaFormat"]["type"] = "game_media";
  }
#endif
...
void UMediaClientImpl::NotifyForeground() {
  ...
  // SystemMusic app is overlay on other foreground apps.
  if (!IsAppName(kSystemMusic)) {
