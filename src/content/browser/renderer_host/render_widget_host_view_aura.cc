...
#if defined(OS_WEBOS)
    // LG system key should not be printed, mark it as system
    NativeWebKeyboardEvent native_web_event(event, event.GetCharacter());
    native_web_event.is_system_key =
        ui::KeycodeConverterWebOS::IsKeyCodeNonPrintable(
            event.GetConflatedWindowsKeyCode());
    // Send a blink::WebInputEvent::Char event to |host_|.
    ForwardKeyboardEventWithLatencyInfo(native_web_event, *event.latency(),
                                        nullptr);
#else
    // Send a blink::WebInputEvent::Char event to |host_|.
