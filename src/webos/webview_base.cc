...
void WebViewBase::ForwardWebOSEvent(WebOSEvent* event) {
  ...
  if (event->GetType() == WebOSEvent::KeyPress ||
    ...
    if (keycode == webos::Key_webOS_CursorShow)
      keycode = VK_LGE_CURSOR_SHOW;
    else if (keycode == webos::Key_webOS_CursorHide)
      keycode = VK_LGE_CURSOR_HIDE;
    else if (keycode == webos::Key_webOS_Back) {
      keycode = VK_LGE_RCU_BACK;
      key_event->SetDomKey(ui::DomKey::GO_BACK);
    }
