...
bool WebView::IsDrmEncrypted(const std::string& url) {
  net::AppDRMFileManagerWrapper adfm = net::AppDRMFileManagerWrapper(url);
  return adfm.Check();
}
...
void WebView::RenderFrameCreated(content::RenderFrameHost* render_frame_host) {
  ...
  if (network_timeout_) {
    ...
    if (client)
      client->SetNetworkQuietTimeout(network_timeout_);
    network_timeout_ = 0.0;
  }
  frame_host_ = render_frame_host;
}
...
void WebView::SetNetworkQuietTimeout(double timeout) {
  if (frame_host_) {
    mojo::AssociatedRemote<mojom::AppRuntimeWebViewClient> client;
    frame_host_->GetRemoteAssociatedInterfaces()->GetInterface(&client);
    if (client)
      client->SetNetworkQuietTimeout(timeout);
  } else {
    network_timeout_ = timeout;
  }
}
...
void WebView::EnterFullscreenModeForTab(
  ...
  SwitchFullscreenModeForTab(web_contents, true);
}
